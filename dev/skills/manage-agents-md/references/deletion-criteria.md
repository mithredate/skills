...
- **It names something that no longer exists.** A path, script, command, package, branch, or file that is not there. Check with `ls`, `git ls-files`, the manifest, `--help`.
- **The environment now states it.** A linter rule, a manifest script or target, a config value. The what belongs to the environment; only the how stays in the file.
