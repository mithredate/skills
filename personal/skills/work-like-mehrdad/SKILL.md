...
- **Craft baseline.** Precise domain-correct names — rename bad ones on sight. Named constants over magic literals; every usage replaced. Configurable over hardcoded env values. Stale comments, dead conditionals, and a CLAUDE.md line the environment contradicts are defects; for the last, call the Skill tool with `dev:manage-claude-md` to housekeep it, never work around it. Comment only when absolutely necessary: comments are liabilities that decay — prefer intention-revealing names. A comment that survives explains only the why; one explaining the what or how is a code smell.
